...
void __attribute__((noinline)) test1(void *q, int x) {
  ...
  if (__builtin_object_size(p, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 3) {
    ;
  }
  if (__builtin_object_size(&a.c[9], 2) != sizeof(a) - __builtin_offsetof(struct A, c) - 9) {
    ;
  }
  ...
  if (__builtin_object_size(r, 2) != (x < 0 ? sizeof(a) - __builtin_offsetof(struct A, a) - 9 : sizeof(a) - __builtin_offsetof(struct A, c) - 1))
    ;
#else
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, c) - 1) {
    ;
  }
#endif
  if (x < 6) {
  ...
  if (__builtin_object_size(&y.b, 2) != sizeof(a) - __builtin_offsetof(struct A, b)) {
    ;
  }
#ifdef __builtin_object_size
  if (__builtin_object_size(r, 2) != (x < 6 ? 2 * sizeof(w[0]) - __builtin_offsetof(struct A, a) - 1 : sizeof(a) - __builtin_offsetof(struct A, a) - 6))
    ;
#else
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 6) {
    ;
  }
#endif
  if (x < 20) {
...
#ifdef __builtin_object_size
  size_t objsz = (x < 30 ? sizeof(a) : sizeof(a) - __builtin_offsetof(struct A, a) - 3);
  if (__builtin_object_size(r, 2) != objsz)
    ;
#else
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 3) {
    ;
  }
#endif
  r = memcpy(r, "a", 2);
  ...
  if (__builtin_object_size(r, 2) != objsz - 4)
    ;
#else
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 3 - 4) {
    ;
  }
#endif
  r = &a.a[4];
  r = memset(r, 'a', 2);
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 4) {
    ;
  }
  r = memset(r + 2, 'b', 2) + 2;
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 8) {
    ;
  }
  r = &a.a[1];
  r = strcpy(r, "ab");
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 1) {
    ;
  }
  r = strcpy(r + 2, "cd") + 2;
  if (__builtin_object_size(r, 2) != sizeof(a) - __builtin_offsetof(struct A, a) - 5) {
    ;
  }
...
void __attribute__((noinline)) test3(void) {
  ...
  if (__builtin_object_size(&x.a[0].b, 2) != sizeof(x) - __builtin_offsetof(struct A, b)) {
    ;
  }
  if (__builtin_object_size(&x.a[1].c, 2) != sizeof(x) - sizeof(struct A) - __builtin_offsetof(struct A, c)) {
    ;
  }
  if (__builtin_object_size(&x.a[1].c[0], 2) != sizeof(x) - sizeof(struct A) - __builtin_offsetof(struct A, c)) {
    ;
  }
  if (__builtin_object_size(&x.a[1].c[3], 2) != sizeof(x) - sizeof(struct A) - __builtin_offsetof(struct A, c) - 3) {
    ;
  }
  if (__builtin_object_size(&x.b, 2) != sizeof(x) - __builtin_offsetof(struct B, b)) {
    ;
  }
  if (__builtin_object_size(&x.b.a, 2) != sizeof(x) - __builtin_offsetof(struct B, b)) {
    ;
  }
  if (__builtin_object_size(&x.b.a[0], 2) != sizeof(x) - __builtin_offsetof(struct B, b)) {
    ;
  }
  if (__builtin_object_size(&x.b.a[3], 2) != sizeof(x) - __builtin_offsetof(struct B, b) - 3) {
    ;
  }
  if (__builtin_object_size(&x.b.b, 2) != sizeof(x) - __builtin_offsetof(struct B, b) - __builtin_offsetof(struct A, b)) {
    ;
  }
  if (__builtin_object_size(&x.b.c, 2) != sizeof(x) - __builtin_offsetof(struct B, b) - __builtin_offsetof(struct A, c)) {
    ;
  }
  if (__builtin_object_size(&x.b.c[0], 2) != sizeof(x) - __builtin_offsetof(struct B, b) - __builtin_offsetof(struct A, c)) {
    ;
  }
  if (__builtin_object_size(&x.b.c[3], 2) != sizeof(x) - __builtin_offsetof(struct B, b) - __builtin_offsetof(struct A, c) - 3) {
    ;
  }
  if (__builtin_object_size(&x.c, 2) != sizeof(x) - __builtin_offsetof(struct B, c)) {
    ;
  }
  if (__builtin_object_size(&x.c[0], 2) != sizeof(x) - __builtin_offsetof(struct B, c)) {
    ;
  }
  if (__builtin_object_size(&x.c[1], 2) != sizeof(x) - __builtin_offsetof(struct B, c) - 1) {
    ;
  }
  if (__builtin_object_size(&x.d, 2) != sizeof(x) - __builtin_offsetof(struct B, d)) {
    ;
  }
  if (__builtin_object_size(&x.e, 2) != sizeof(x) - __builtin_offsetof(struct B, e)) {
    ;
  }
  if (__builtin_object_size(&x.f, 2) != sizeof(x) - __builtin_offsetof(struct B, f)) {
    ;
  }
  dp = &__real__ x.f;
  if (__builtin_object_size(dp, 2) != sizeof(x) - __builtin_offsetof(struct B, f)) {
    ;
  }
  dp = &__imag__ x.f;
  if (__builtin_object_size(dp, 2) != sizeof(x) - __builtin_offsetof(struct B, f) - sizeof(x.f) / 2) {
    ;
  }
  dp = &y;
...
void __attribute__((noinline)) test8(void) {
  ...
  q = q - 8;
  if (__builtin_object_size(q, 2) != 0 && __builtin_object_size(q, 2) != sizeof(t) - 10 + 8) {
    ;
  }
  p = &t.buf[-4];
  if (__builtin_object_size(p, 2) != 0) {
...
#ifdef __builtin_object_size
  if (__builtin_object_size(p, 2) != sizeof(y) - __builtin_offsetof(struct A, c) - 8 + cond)
    ;
#else
  if (__builtin_object_size(p, 2) != 0) {
    ;
