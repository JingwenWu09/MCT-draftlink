...
__attribute__((noinline, noclone)) static void test_func(double *a, int L, int m, int n, int N) {
  int i, k;
  for (i = 0; i < N; i++) {
    k = i - m;
    if (k >= 0 && k < n) {
...
int main() {
  ...
  p = mmap((void *)0, L * n * sizeof(double), PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (p == MAP_FAILED) {
    return 0;
  }
