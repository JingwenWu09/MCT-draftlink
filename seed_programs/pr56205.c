...
__attribute__((noinline, noclone)) static void f1(const char *fmt, ...) {
  ...
  va_start(ap, fmt);
  if (__builtin_strcmp(va_arg(ap, const char *), "foo") != 0 || va_arg(ap, int) != 1 || __builtin_strcmp(va_arg(ap, const char *), "bar") != 0) {
    __builtin_abort();
  }
  va_end(ap);
}
...
__attribute__((noinline, noclone)) static void f2(const char *fmt, va_list ap) {
  asm volatile("" : : : "memory");
  if (__builtin_strcmp(fmt, "baz") != 0 || __builtin_strcmp(va_arg(ap, const char *), "foo") != 0 || va_arg(ap, double) != 12.0 || va_arg(ap, int) != 26) {
    __builtin_abort();
  }
}
