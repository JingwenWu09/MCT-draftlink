...
The assert() functions are true, which is confirmed by compiling with gcc and clang, while
cbmc gives the FAILURE result towards assert(labs(pa-pb) == 0), assert(labs(pa-pb) == 0)
and assert(abs(i) >= 0) which means the assertions are false. abs(i) is always true,
