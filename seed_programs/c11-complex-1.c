...
int main(void) {
  res_f = num_f / den_f;
  if (!isnan(__real__ res_f) || !isnan(__imag__ res_f) || !isnan(__real__ cres_f) || !isnan(__imag__ cres_f)) {
    abort();
  }
  res_d = num_d / den_d;
  if (!isnan(__real__ res_d) || !isnan(__imag__ res_d) || !isnan(__real__ cres_d) || !isnan(__imag__ cres_d)) {
    abort();
  }
  res_ld = num_ld / den_ld;
  if (!isnan(__real__ res_ld) || !isnan(__imag__ res_ld) || !isnan(__real__ cres_ld) || !isnan(__imag__ cres_ld)) {
    abort();
  }
  exit(0);
}
