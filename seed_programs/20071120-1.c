...
void __attribute__((noinline, noreturn)) vec_assert_fail(void) {
  abort();
}
...
static __inline__ deferred_access *VEC_deferred_access_base_last(VEC_deferred_access_base *vec_) {
  (void)((vec_ && vec_->num) ? 0 : (vec_assert_fail(), 0));
  return &vec_->vec[vec_->num - 1];
}

static __inline__ void VEC_deferred_access_base_pop(VEC_deferred_access_base *vec_) {
  (void)((vec_->num) ? 0 : (vec_assert_fail(), 0));
  --vec_->num;
}

void __attribute__((noinline)) perform_access_checks(VEC_deferred_access_check_gc *p) {
  abort();
}
...
const struct ggc_root_tab gt_pch_rs_gt_cp_semantics_h[] = {{&deferred_access_no_check}};
