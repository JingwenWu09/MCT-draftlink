...
public class DataFlowSR {
	...
	public void Transformation(File file, String mutationDir) {
		...
		for(FunctionBlock fun: functionBlockList) {
			...
			//mutationCaseTOne addedStatementsList,分别存放的是vardeclare、expStmt、conditionList
			if(initializedVars.size() == 1 ) {	//只有一个，再生成一个变量
				addLine = firstAssignedLine.get(0);
				...
				vars.add(new String[] {initializedVars.get(0).getType(), initializedVars.get(0).getName()});
				...
				for( int start = 0; start <= initializedVars.size() - 2; start++) {		//大于等于2个，窗口为2，保证每个变量都能使用到
					int end = start + 2;
					...
					addedStatementsList = genSRModPow(existVars);
					addedStatementsList.get(0).addAll(varDeclare);
					genMutationCaseOne(firstFunctionStartline, addedStatementsList, fun.leftBraceLine, addLine);
				}
	...
	public void genMutationCaseOne(int firstFunctionStartline, List<List<String>> addedStatementsList, int funLeftBraceLine, int addLine) {
		try {
