...
void foo(struct U *__restrict p1, struct U *__restrict p2, struct S l1, struct S l2, struct S l3, struct S l4, const double _Complex *__restrict x, int y, int z) {
  int i, j;
  while (y < z - 2) {
    for (j = 0; j < 5; ++j) {
...
int main() {
  ...
  for (j = 0; j < 5; ++j) {
    for (i = 0; i < 3; ++i) {
      if (p1.j[j].r.v[i] != 3752430 + j * 1494.0 || p1.j[j].i.v[i] != p1.j[j].r.v[i] * 2 || p2.j[j].r.v[i] != 2502450 + j * 996.0 || p2.j[j].i.v[i] != p2.j[j].r.v[i] * 2) {
        abort();
      }
    }
  }
