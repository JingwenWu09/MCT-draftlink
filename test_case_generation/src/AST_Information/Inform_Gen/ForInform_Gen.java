...
public class ForInform_Gen{
	
	public List<ForStatement> outmostForList = new ArrayList<ForStatement>();
	
	public ForInform_Gen(AstInform_Gen astGen) {
		getForInform(astGen);
	}
	
	public void getForInform(AstInform_Gen astGen) {
		//astList
		...
		Stack<ForStatement> forStack = new Stack<ForStatement>();
...
//		String regexVarDecl = "(Parm)?VarDecl\\s(0x[0-9a-z]+)\\s<.*>.*?(\\bused\\b)?\\s([_a-zA-Z]+[_a-zA-Z0-9]*)\\s'([_a-zA-Z]+[_a-zA-Z0-9,\\.\\s\\(\\)\\*\\[\\]]*)'"; 
		...
		int topEndLine = 0;
		ForStatement newStmt = null;
		ForStatement topStmt = null;
		ForStatement popStmt = null;
		...
		for(String line: astList) {
			...
			while(!forStack.isEmpty()) {	//考虑两层嵌套在同一行结束的情况，for(){...for(){}}
				topStmt = forStack.peek();
				...
				//当前操作行不属于栈顶元素内部，弹出栈顶元素
					popStmt = forStack.pop();
					popStmt.getForList().sort(new SortByStartLine());
					/**/
					if(forStack.isEmpty()) {
						//弹出popStmt后，栈为空，说明popStmt是最外层For循环
						outmostForList.add(popStmt);
					}
				}
			}
			...
			if(mVarUse.find()) {
				...
				ForStatement childLoop, parentLoop;
				...
				if(!forStack.isEmpty()) {
					...
					childLoop = forStack.peek();
					parentLoop = childLoop.getParentForStmt();
			...
			if(flagCompound == true) {
				if(!astStmtStack.isEmpty()) {
					topAstStmt = astStmtStack.peek();
					if(topAstStmt.getStmtType().equals("ForStmt")) {
						topStmt = forStack.peek();
			...
			if(flagNewFor == true) {
				newStmt = new ForStatement();
				newStmt.setStartLine(startLine);
				newStmt.setEndLine(endLine);
				if(!forStack.isEmpty()) {
					topStmt = forStack.peek();
					topStmt.getForList().add(newStmt);
					newStmt.setParentForStmt(topStmt);
					System.out.println("has inner layer!");
				}
				forStack.push(newStmt);
			}
		}
	...
	private static class SortByStartLine implements Comparator<ForStatement>{
		@Override
        public int compare(ForStatement stmt1, ForStatement stmt2) {
