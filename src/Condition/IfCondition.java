...
public class IfCondition {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		File file = new File("/home/jing/桌面/IfRedone/vla-4.c");
		String mutationDir = "/home/jing/桌面/IfRedone";
		IfCondition condition = new IfCondition();
		condition.Transformation(file, mutationDir);
//		long successCnt = RunAssert.assertMutation(new File(mutationDir), 0, new File(mutationDir));
...
//				rightCondition = rightCondition.equals("")? "0": rightCondition;
				...
				initial_block.add(ifSline + "if( " + leftCondition + " ) {");
				trans_block.add(ifSline + "if( " + leftCondition + " || " + rightCondition + " ) {");
				for(int i = stmt.getStartLine()+1; i < stmt.getElseline(); i++) {
					initial_block.add(initialList.get(i));
					trans_block.add(initialList.get(i));
				}
				
				initial_block.add(space + "} else if( " + rightCondition + " ) {");
				for( int i = stmt.getStartLine()+1; i < stmt.getElseline(); i++) {
					initial_block.add(initialList.get(i));
				}
...
//				rightCondition = rightCondition.equals("")? "1": rightCondition;
				...
				initial_block.add(ifSline + "if( " + leftCondition + " ) {");
				initial_block.add(space + "\tif( " + rightCondition + " ) {");
				trans_block.add(ifSline + "if( " + leftCondition + " && " + rightCondition + " ) {");
//				for( int i = stmt.getStartLine()+1; i < stmt.getElseline(); i++) {
//					initial_block.add(initialList.get(i));
//					trans_block.add("\t" + initialList.get(i));
