...
import inversion_operation.Inversion;
import iv_operation.*;
import loopFusion.Fusion;
import loopInvariant.Invariant;
import loopUnswitching.Unswitching;
import unrolling_operation.Unrolling;
...
//			if(filename.equals("use-after-scope-switch-3.c") == false) continue ;
			...
			else if(mutationType.equals("Fusion")) {
				Fusion fusion = new Fusion();
				fusion.Transformation(file, mutationDir);
			}
			else if(mutationType.equals("Invariant")) {
				Invariant invariant = new Invariant();
				invariant.Transformation(file, mutationDir);
			}
			else if(mutationType.equals("Inversion")) {
				Inversion inversion = new Inversion();
				inversion.inversionTransformation(file, mutationDir);
			}
			else if(mutationType.equals("IVElimination")) {
				IVE_Exponent ib = new IVE_Exponent(file, mutationDir);
			}
			else if(mutationType.equals("Unrolling")) {
				Unrolling unrolling = new Unrolling();
//				System.out.println("111");
				unrolling.unrollingTransformation(file, mutationDir);
//				System.out.println("222\n");
			}
			else if(mutationType.equals("Unswitching")) {
				Unswitching unswitching = new Unswitching();
				unswitching.Transformation(file, mutationDir);
			}
