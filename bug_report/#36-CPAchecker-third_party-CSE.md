# Bug #9 in CPAchecker was confirmed as a third-party dependency related issue. It was exposed by a test case generated using common subexpression elimination transformation. 
...
int main() {
  ...
  if (*m == '\0') {
    ...
    if (k < o) {
      ...
      int flag_orig = 0;
      if (((k + n) * (q - k)) > f->a5 && ((k + n) * (q - k)) < q) {
        flag_orig = 1;
      }

      int flag_new = 0;
      unsigned long common = (k_1 + n_1) * (q_1 - k_1); 
      if (common > f_1->a5 && common < q_1) {
        flag_new = 1;
      }

      assert(flag_orig == flag_new);
      assert(p.d1 == p_1.d1) ;
      assert(p.d2 == p_1.d2) ;
      assert(p.d4 == p_1.d4) ;
      assert(p.d5 == p_1.d5) ;
