...
int main() {
  ...
  f6();
  for (i = 0; i < N; i++) {
    if (g[i] != (signed TYPE1)(a[i] << b[i]) || h[i] != (unsigned TYPE1)(c[i] >> b[i]) || j[i] != (signed TYPE2)(d[i] << e[i]) || k[i] != (unsigned TYPE2)(f[i] >> e[i])) {
      abort();
    }
  }
  f2();
  ...
  f9();
  for (i = 0; i < N; i++) {
    if (g[i] != (signed TYPE1)(a[i] >> b[i]) || j[i] != (signed TYPE2)(d[i] >> e[i]) || k[i] != (unsigned TYPE2)(f[i] >> b[i])) {
      abort();
    }
  }
  f7();
